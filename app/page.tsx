...
export default function Home() {
  ...
  return <main id="main">
    ...
    <section className="works-preview">
      <div className="wrap section-heading"><p className="section-no">02 / Selected Works</p><h2>Recent <em>work</em></h2></div>
      {featured.length ? <div className="art-grid wrap">{featured.map((art) => <ArtworkCard key={art.slug} artwork={art} />)}</div> : <div className="empty-art wrap"><div className="empty-circle" aria-hidden="true" /><p>The collection is being prepared.<br />Artwork images will appear here soon.</p></div>}
    ...
    <section className="exhibition-preview wrap">
      <div className="section-heading"><p className="section-no">04 / Exhibitions</p><h2>Solo <em>chapters</em></h2></div>
      {siteContent.exhibitions.slice(0, 2).map((exhibition, index) => <Link href="/exhibitions" className="ex-row" key={exhibition.title}>
        <span>0{index + 1}</span>
        {exhibition.poster && <div className="ex-poster"><Image unoptimized src={exhibition.poster} alt={`${exhibition.title} official exhibition poster`} width={360} height={480} sizes="(max-width: 800px) 92px, 180px" /></div>}
        <h3>{exhibition.amharic}<small>{exhibition.title}</small></h3>
        <p>{exhibition.date}<br />{exhibition.venue}</p>
        <b>↗</b>
      </Link>)}
    </section>

    <section className="contact-band">
      <div className="wrap"><p className="eyebrow">Conversation / Collaboration / Enquiries</p><h2>Let’s begin a <em>conversation.</em></h2><a className="button button-light" href={`mailto:${siteContent.email}`}>Write to Yofset <span>↗</span></a></div>
    </section>
  </main>;
}
